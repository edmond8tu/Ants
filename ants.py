    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 11
                if not self.ant.can_contain(insect) and not insect.can_contain(self.ant):
                    assert self.ant is None, 'Two ants in {0}'.format(self)
                else:
                    if self.ant.can_contain(insect):
                        self.ant.contain_ant(insect)
                        #print(self.ant.ant, insect)
                    else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while temp_place is not hive:
            if len(temp_place.bees) != 0 and (count <= self.max_range and count >= self.min_range): 
...
class HungryAnt(Ant):
    ...
    def __init__(self, time_to_digest=3, digesting=0):
        # BEGIN Problem 10
    ...
    def action(self, colony):
        # BEGIN Problem 10
        #print(self.time_to_digest)
        if self.digesting == 0:
            rand_bee = random_or_none(self.place.bees)
            if rand_bee is not None:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            temp_place = self.place
            #print(self.place.exit.exit.exit.ant)
            while temp_place.exit is not None:
                temp_place = temp_place.exit
                if temp_place.ant is not None:
                    if temp_place.ant.container and temp_place.ant.ant is not None:
                        if temp_place.ant.ant not in self.buff_ants:
                            temp_place.ant.ant.damage *= 2
                            self.buff_ants.append(temp_place.ant.ant)
                    elif temp_place.ant not in self.buff_ants:
                        temp_place.ant.damage *= 2
                        self.buff_ants.append(temp_place.ant)
                

                """if temp_place.ant is not None and temp_place.ant not in self.buff_ants:
                    if temp_place.ant.container and temp_place.ant.ant is not None:
                        temp_place.ant.ant.damage *= 2
                        self.buff_ants.append(temp_place.ant.ant)
                    else:
